...
class XcodeSpec: QuickSpec {
  override func spec() {
    // The fixture is maintained at https://github.com/ikesyo/utica-fixtures-ReactiveCocoaLayout
        ...
        .flatMap(.merge) { binaryURL -> SignalProducer<Bool, CarthageError> in
          ...
          return nmTask.launch()
            ...
            .flatMap(.merge) { output -> SignalProducer<Bool, Never> in
              SignalProducer(value: output.contains("SO "))
            }
        }.single()
    ...
    it("should build static library and place result to subdirectory") {
      ...
      let result = buildInDirectory(_directoryURL, withOptions: BuildOptions(
        configuration: "Debug",
        platforms: [.iOS],
      ...
      let result2 = buildInDirectory(_directoryURL, withOptions: BuildOptions(
        configuration: "Debug",
        platforms: [.iOS],
        derivedDataPath: Constants.Dependency.derivedDataURL.appendingPathComponent("TestFramework-o2nfjkdsajhwenrjle").path
      ), rootDirectoryURL: directoryURL)
