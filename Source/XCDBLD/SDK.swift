...
public struct SDK: Hashable {
  ...
  public var platformSimulatorlessFromHeuristic: String {
    guard rawValue != "macosx" else { return "Mac" }

    guard simulatorHeuristic.isEmpty == false else {
      let result = SDK.knownIn2019YearDictionary[rawValue, default: ("", [""], "")].1
      guard result.first != .some("") else {
        return ["simulator"].reduce(into: name) {
...
public extension SDK {
  ...
  static let setFromJSONShowSDKs: SignalProducer<Set<SDK>?, Never> =
    Task("/usr/bin/xcrun", arguments: ["xcodebuild", "-showsdks", "-json"])
    .launch()
    .materializeResults() // to map below and ignore errors
    .compactMap { try? JSONSerialization.jsonObject(with: $0.value?.value ?? Data([]), options: JSONSerialization.ReadingOptions()) as? NSArray ?? NSArray() }
    .map {
...
extension SDK: CustomStringConvertible {
  public var description: String {
    return SDK.knownIn2019YearDictionary[rawValue]?.1.first!
      ?? platformSimulatorlessFromHeuristic.appending(isSimulator ? " Simulator" : "")
  }
}
