...
extension String {
  ...
  /// complete.
  var linesProducer: SignalProducer<String, Never> {
...
private extension Signal {
  ...
  func permute<U>(with otherSignal: Signal<U, Error>) -> Signal<(Value, U), Error> {
    // swiftlint:disable:previous cyclomatic_complexity function_body_length
    return Signal<(Value, U), Error> { observer, lifetime in
      let lock = NSLock()
      lock.name = "org.utica.UticaKit.permute"
...
extension SignalProducer {
  ...
  internal func succeeded() -> SignalProducer<Bool, Never> {
    return then(SignalProducer<Bool, Never>(value: true))
...
extension SignalProducer where Value: SignalProducerProtocol, Error == Value.Error {
  ...
  func permute() -> SignalProducer<[Value.Value], Error> {
    return collect()
...
extension Signal where Value: EventProtocol, Value.Error == Error {
  ...
  func dematerializeErrorsIfEmpty() -> Signal<Value.Value, Error> {
...
extension SignalProducer where Value: EventProtocol, Value.Error == Error {
  ...
  /// Error events if no values were sent.
  func dematerializeErrorsIfEmpty() -> SignalProducer<Value.Value, Error> {
...
extension Scanner {
  ...
  var currentLine: String {
    ...
    let scanRange = NSRange(location: scanLocation, length: 0)
...
extension URL {
  ...
  public func hasSubdirectory(_ possibleSubdirectory: URL) -> Bool {
    let standardizedSelf = standardizedFileURL
  ...
  fileprivate func volumeSupportsFileCloning() throws -> Bool {
    ...
    let values = try resourceValues(forKeys: [key]).allValues
  ...
  internal func swiftHeaderURL() -> URL? {
    let headersURL = appendingPathComponent("Headers", isDirectory: true).resolvingSymlinksInPath()
  ...
  internal func swiftmoduleURL() -> URL? {
    let headersURL = appendingPathComponent("Modules", isDirectory: true).resolvingSymlinksInPath()
    let dirContents = try? FileManager.default.contentsOfDirectory(at: headersURL, includingPropertiesForKeys: [], options: [])
...
extension FileManager: ReactiveExtensionsProvider {
  ...
  internal func copyItem(at from: URL, to: URL, avoiding·rdar·32984063: Bool) throws {
    guard avoiding·rdar·32984063, try from.volumeSupportsFileCloning() else {
      return try copyItem(at: from, to: to)
    }
...
public extension Reactive where Base: FileManager {
  ...
  /// function.
  func createTemporaryDirectoryWithTemplate(_ template: String) -> SignalProducer<URL, CarthageError> {
...
extension Reactive where Base: URLSession {
  ...
  ///         200...299 is received).
  func download(with request: URLRequest) -> SignalProducer<(URL, URLResponse), AnyError> {
    return SignalProducer { [base = self.base] observer, lifetime in
