...
extension SemanticVersion {
  ...
  public static func from(_ pinnedVersion: PinnedVersion) -> Result<SemanticVersion, ScannableError> {
    ...
    return from(scanner).flatMap { version in
      if scanner.isAtEnd {
...
extension SemanticVersion: Scannable {
  ...
  public static func from(_ scanner: Scanner) -> Result<SemanticVersion, ScannableError> {
    ...
    guard scanner.scanCharacters(from: versionCharacterSet, into: &versionBuffer),
          let version = versionBuffer as String?
    else {
  ...
  private static func validateBuildMetadata(_ buildMetadata: String, fullVersion: String) -> ScannableError? {
  ...
  private static func validatePreRelease(_ preRelease: String, fullVersion: String) -> ScannableError? {
...
extension Scanner {
  ...
  fileprivate func scanStringWithPrefix(_ prefix: Character, until: String) -> String? {
    guard !isAtEnd, remainingSubstring?.first == prefix else { return nil }

    var buffer: NSString?
    scanUpTo(until, into: &buffer)
  ...
  var remainingSubstring: Substring? {
    return Range(
      NSRange(
        location: scanLocation /* our UTF-16 offset */,
        length: (string as NSString).length - scanLocation
      ),
      in: string
    ).map {
      self.string[$0]
...
extension SemanticVersion {
  ...
  func hasSameNumericComponents(version: SemanticVersion) -> Bool {
    return components == version.components
  }
}
...
extension String {
  ...
  private var numericValue: Int? {
    if !isEmpty, rangeOfCharacter(from: SemanticVersion.semVerDecimalDigits.inverted) == nil {
      return Int(self)
    }
    return nil
  }
  ...
  fileprivate func lesserThanPreReleaseVersionComponent(other: String) -> Bool {
    ...
    guard let numericSelf = numericValue else {
...
public extension VersionSpecifier {
  /// Defines an order of strictness.
  func isStricter(than specifier: VersionSpecifier) -> Bool {
