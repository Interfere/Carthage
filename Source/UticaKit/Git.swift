...
public func cloneSubmoduleInWorkingDirectory(_ submodule: Submodule, _ workingDirectoryURL: URL) -> SignalProducer<Void, CarthageError> {
  ...
  let purgeGitDirectories = FileManager.default.reactive
    ...
    .attemptMap { enumerator, url -> Result<Void, CarthageError> in
      repositoryCheck("enumerate name of descendant at \(url.path)", attempt: {
...
/// directory.
private func checkoutSubmodule(_ submodule: Submodule, _ submoduleWorkingDirectoryURL: URL) -> SignalProducer<Void, CarthageError> {
...
internal func list(treeish: String, atPath path: String, inRepository repositoryURL: URL) -> SignalProducer<String, CarthageError> {
  ...
  .flatMap(.merge) { (output: String) -> SignalProducer<String, CarthageError> in
    SignalProducer(output.lazy.split(separator: "\0").map(String.init))
  }
}
...
internal func gitmodulesEntriesInRepository(
  ...
  return launchGitTask(baseArguments + ["--get-regexp", "submodule\\..*\\.path"], repositoryFileURL: repositoryFileURL)
    ...
    .flatMap(.concat) { name, path -> SignalProducer<(name: String, path: String, url: GitURL), CarthageError> in
      launchGitTask(baseArguments + ["--get", "submodule.\(name).url"], repositoryFileURL: repositoryFileURL)
...
public func submodulesInRepository(_ repositoryFileURL: URL, revision: String = "HEAD") -> SignalProducer<Submodule, CarthageError> {
  return isGitRepository(repositoryFileURL)
    ...
    .flatMap(.concat) { actualRepoURL in
      gitmodulesEntriesInRepository(repositoryFileURL, revision: revision)
        .flatMap(.concat) { name, path, url in
          submoduleSHAForPath(actualRepoURL, path, revision: revision)
...
private func ensureDirectoryExistsAtURL(_ fileURL: URL) -> SignalProducer<Void, CarthageError> {
...
public func addSubmoduleToRepository(_ repositoryFileURL: URL, _ submodule: Submodule, _ fetchURL: GitURL) -> SignalProducer<Void, CarthageError> {
  ...
  return isGitRepository(submoduleDirectoryURL)
    .map { isRepository in
      ...
      isRepository && FileManager.default.fileExists(atPath: submoduleDirectoryURL.appendingPathComponent(".git").path)
    }
    .flatMap(.merge) { submoduleExists -> SignalProducer<Void, CarthageError> in
      if submoduleExists {
        ...
        // This can happen if you first do `utica checkout` and then try it again with
        // `--use-submodules`, e.g.
