...
public struct VersionFile: Codable {
  ...
  public init?(url: URL) {
    guard
      ...
      let versionFile = try? JSONDecoder().decode(VersionFile.self, from: jsonData)
    else {
      return nil
    }
    self = versionFile
        ...
        if !isSwiftFramework(frameworkURL) {
          ...
          return frameworkSwiftVersion(frameworkURL)
            .map { swiftVersion -> Bool in
              swiftVersion == localSwiftVersion || isModuleStableAPI(localSwiftVersion, swiftVersion, frameworkURL)
            }
  ...
  public func write(to url: URL) -> Result<Void, CarthageError> {
    ...
    return SignalProducer<URL, CarthageError>(buildProducts)
      ...
      .reduce(into: platformCaches) { (platformCaches: inout [String: [CachedFramework]], values: (String, FrameworkDetail)) in
        ...
        switch values.1.frameworkLocator {
          case let .platformDirectory(name: name, linking: linking):
            platformName = name
          ...
          case let .xcframework(name: container, libraryIdentifier: identifier):
      ...
      .flatMap(.merge) { platformCaches -> SignalProducer<Void, CarthageError> in
        createVersionFile(
          commitish,
...
public func versionFileMatches(
  ...
  return swiftVersion(usingToolchain: toolchain)
    ...
    .flatMap(.concat) { localSwiftVersion in
      SignalProducer<SDK, CarthageError>(platformsToCheck)
        .flatMap(.merge) { platform in
          versionFile.satisfies(
            platform: platform,
            commitish: commitish,
