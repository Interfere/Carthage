...
private func determineSwiftVersion(usingToolchain toolchain: String?) -> SignalProducer<String, SwiftVersionError> {
  ...
  return taskDescription.launch(standardInput: nil)
    ...
    .map { data -> String? in
      parseSwiftVersionCommand(output: String(data: data, encoding: .utf8))
    }
...
internal func checkSwiftFrameworkCompatibility(_ frameworkURL: URL, usingToolchain toolchain: String?) -> SignalProducer<Void, SwiftVersionError> {
  return SignalProducer.combineLatest(swiftVersion(usingToolchain: toolchain), frameworkSwiftVersion(frameworkURL))
    .attemptMap { localSwiftVersion, frameworkSwiftVersion in
      localSwiftVersion == frameworkSwiftVersion || isModuleStableAPI(localSwiftVersion, frameworkSwiftVersion, frameworkURL)
        ? .success(())
...
internal func isModuleStableAPI(
  _ localSwiftVersion: String,
  ...
  let hasSwiftInterfaceFile = try? FileManager.default.contentsOfDirectory(
    at: swiftModuleURL,
    includingPropertiesForKeys: nil,
    options: []
  ).first { url -> Bool in
    url.lastPathComponent.contains("swiftinterface")
  } != nil
...
/// Creates a task description for executing `xcodebuild` with the given
/// arguments.
public func xcodebuildTask(_ task: String, _ buildArguments: BuildArguments, environment _: [String: String]? = nil) -> Task {
...
public func buildableSchemesInDirectory( // swiftlint:disable:this function_body_length
  ...
  return locator
    ...
    .flatMap(.merge) { (projects: [(ProjectLocator, [Scheme])]) -> SignalProducer<(Scheme, ProjectLocator), CarthageError> in
      schemesInProjects(projects).flatten()
    }
    ...
    .flatMap(.concurrent(limit: 4)) { scheme, project -> SignalProducer<(Scheme, ProjectLocator), CarthageError> in
      locator
        ...
        .start(on: QueueScheduler(qos: .default, name: "org.utica.UticaKit.Xcode.buildInDirectory"))
...
public func schemesInProjects(_ projects: [(ProjectLocator, [Scheme])]) -> SignalProducer<[(Scheme, ProjectLocator)], CarthageError> {
  return SignalProducer<(ProjectLocator, [Scheme]), CarthageError>(projects)
    ...
    .flatMap(.concat) { project, schemes in
      SignalProducer<(Scheme, ProjectLocator), CarthageError>(schemes.map { ($0, project) })
    }
    .collect()
}
...
private func copyBuildProductIntoDirectory(_ directoryURL: URL, _ settings: BuildSettings) -> SignalProducer<URL, CarthageError> {
  ...
  return SignalProducer(result: target.fanout(settings.wrapperURL))
    .flatMap(.merge) { target, source in
      copyProduct(source.resolvingSymlinksInPath(), target)
    }
    .flatMap(.merge) { url in
      copyBCSymbolMapsForBuildProductIntoDirectory(directoryURL, settings)
...
/// the specified URL.
private func mergeExecutables(_ executableURLs: [URL], _ outputURL: URL) -> SignalProducer<Void, CarthageError> {
  ...
  return copyBuildProductIntoDirectory(destinationFolderURL, deviceBuildSettings)
    .flatMap(.merge) { productURL -> SignalProducer<URL, CarthageError> in
      ...
      let mergeProductBinaries = SignalProducer(result: executableURLs.fanout(outputURL))
        .flatMap(.concat) { (executableURLs: [URL], outputURL: URL) -> SignalProducer<Void, CarthageError> in
          mergeExecutables(
      ...
      let mergeProductSwiftHeaderFilesIfNeeded = SignalProducer.zip(simulatorBuildSettings.executableURL, deviceBuildSettings.executableURL, outputURL)
        .flatMap(.concat) { (simulatorURL: URL, deviceURL: URL, outputURL: URL) -> SignalProducer<Void, CarthageError> in
      ...
      let mergeProductModules = SignalProducer.zip(sourceModulesURL, destinationModulesURL)
        .flatMap(.merge) { (source: URL, destination: URL) -> SignalProducer<URL, CarthageError> in
          mergeModuleIntoModule(source, destination)
        }
...
private func mergeIntoXCFramework(in directoryURL: URL, settings: BuildSettings) -> SignalProducer<URL, CarthageError> {
  ...
  let createTemporaryDirectory = fileManager.reactive.createTemporaryDirectoryWithTemplate("utica-xcframework-XXXXXX")
  ...
  return SignalProducer.combineLatest(
    ...
    .attempt { replacementURL in
      Result(at: xcframeworkURL) { url in
    ...
    .flatMap(.concat) { sdk -> SignalProducer<SDK, CarthageError> in
      ...
      return BuildSettings
        ...
        .filter { settings in
          ...
          settings.bitcodeEnabled.value == true || !["appletvos", "watchos"].contains(sdk.rawValue)
        }
        .map { _ in sdk }
    }
    ...
    .flatMapTaskEvents(.concat) { builtProductURL -> SignalProducer<URL, CarthageError> in
      ...
      return UUIDsForFramework(builtProductURL)
        ...
        .flatMap(.concat) { _ -> SignalProducer<TaskEvent<URL>, CarthageError> in
          createDebugInformation(builtProductURL)
        }
...
func extractXCFrameworks(in buildDirectory: URL, for settings: BuildSettings) -> SignalProducer<URL?, CarthageError> {
  ...
  guard let platformTripleOS = settings.platformTripleOS.value,
        ...
        frameworkSearchPaths.contains(where: isRelativeToBuildDirectory)
  else {
  ...
  let makeTemporaryDirectory = SignalProducer<URL, CarthageError> { () -> Result<URL, CarthageError> in
    var templatePath = (NSTemporaryDirectory() as NSString).appendingPathComponent("utica-xcframework-XXXX").utf8CString
    ...
    .flatMap(.concat) { destination -> SignalProducer<TaskEvent<BuildSettings>, CarthageError> in
      ...
      return BuildSettings.load(with: argsForLoading, for: xcodebuildAction)
        .filter { settings in
          ...
          // Do not copy build products that originate from the current project's own utica dependencies
  ...
  return BuildSchemeProducer { observer, lifetime in
    ...
    .flatMap(.concat) { (scheme: Scheme, project: ProjectLocator) -> SignalProducer<TaskEvent<URL>, CarthageError> in
      ...
      let wrappedSDKFilter: SDKFilterCallback = { sdks, scheme, configuration, project in
        sdkFilter((options.platforms /* allow list */ ?? sdks).intersection(sdks), scheme, configuration, project)
      }
    ...
    .collectTaskEvents()
    .flatMapTaskEvents(.concat) { (urls: [URL]) -> SignalProducer<Void, CarthageError> in
...
public func copyAndStripFramework(
  ...
  return SignalProducer.combineLatest(copyProduct(source, target), codesignIdentityQuery, strippedArchitectureData)
    .flatMap(.merge) { _, codesigningIdentity, strippedArchitectureData -> SignalProducer<Void, CarthageError> in
      SignalProducer(value: strippedArchitectureData.1.relativePath)
        .attemptMap {
          Result(at: target.appendingPathComponent($0)) {
...
public func stripDSYM(_ dSYMURL: URL, keepingArchitectures: [String]) -> SignalProducer<Void, CarthageError> {
...
private func stripBinary(_ packageURL: URL, keepingArchitectures: [String]) -> SignalProducer<Void, CarthageError> {
  return architecturesInPackage(packageURL)
    .flatMap(.race) { (packageArchs: [String]) in
      stripArchitectures(packageURL, Set(packageArchs).subtracting(keepingArchitectures))
        .then(SignalProducer<Void, CarthageError>(value: ()))
    }
}
...
public func copyProduct(_ from: URL, _ to: URL) -> SignalProducer<URL, CarthageError> { // swiftlint:disable:this identifier_name
  return SignalProducer<URL, CarthageError> { () -> Result<URL, CarthageError> in
    ...
    func result(allowingErrorCode code: Int, _ result: Result<Void, CarthageError>) -> Result<Void, CarthageError> {
      if case let .failure(.writeFailed(_, error?)) = result, error.code == code {
        return .success(())
      }
      return result
    }
...
public extension SignalProducer where Value == URL, Error == CarthageError {
...
public func nonDestructivelyStripArchitectures(_ frameworkURL: URL, _ architectures: Set<String>) -> SignalProducer<(Data, URL), CarthageError> {
  return SignalProducer(value: frameworkURL)
    ...
    .zip(with: FileManager.default.reactive.createTemporaryDirectoryWithTemplate("utica-lipo-XXXXXX"))
...
private func stripArchitectures(_ packageURL: URL, _ architectures: Set<String>) -> SignalProducer<Void, CarthageError> {
  return SignalProducer<URL, CarthageError> { () -> Result<URL, CarthageError> in binaryURL(packageURL) }
    .flatMap(.merge) { binaryURL -> SignalProducer<Void, CarthageError> in
      ...
      guard architectures.count > 0 else {
        return SignalProducer<Void, CarthageError>.empty
      }
...
public func stripDebugSymbols(_ frameworkURL: URL) -> SignalProducer<Void, CarthageError> {
...
public func stripHeadersDirectory(_ frameworkURL: URL) -> SignalProducer<Void, CarthageError> {
  return stripDirectory(named: "Headers", of: frameworkURL)
}

/// Strips `PrivateHeaders` directory from the given framework.
public func stripPrivateHeadersDirectory(_ frameworkURL: URL) -> SignalProducer<Void, CarthageError> {
  return stripDirectory(named: "PrivateHeaders", of: frameworkURL)
}

/// Strips `Modules` directory from the given framework.
public func stripModulesDirectory(_ frameworkURL: URL) -> SignalProducer<Void, CarthageError> {
  return stripDirectory(named: "Modules", of: frameworkURL)
}

private func stripDirectory(named directory: String, of frameworkURL: URL) -> SignalProducer<Void, CarthageError> {
...
public func BCSymbolMapsForFramework(_ frameworkURL: URL) -> SignalProducer<URL, CarthageError> {
  ...
  return UUIDsForFramework(frameworkURL)
    ...
    .map { uuid in
      directoryURL.appendingPathComponent(uuid.uuidString, isDirectory: false).appendingPathExtension("bcsymbolmap")
    }
}
...
private func codesign(_ frameworkURL: URL, _ expandedIdentity: String) -> SignalProducer<Void, CarthageError> {
