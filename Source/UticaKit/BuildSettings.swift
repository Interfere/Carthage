...
public struct BuildSettings {
  ...
  public var builtProductsDirectoryURL: Result<URL, CarthageError> {
    return self["BUILT_PRODUCTS_DIR"].map { productsDir in
      URL(fileURLWithPath: productsDir, isDirectory: true)
    }
  }
  ...
  public var executableURL: Result<URL, CarthageError> {
    return productsDirectoryURLDependingOnAction
      ...
      .map { productsDirectoryURL, executablePath in
        productsDirectoryURL.appendingPathComponent(executablePath)
      }
  }
  ...
  public var wrapperURL: Result<URL, CarthageError> {
    return productsDirectoryURLDependingOnAction
      ...
      .map { productsDirectoryURL, wrapperName in
        productsDirectoryURL.appendingPathComponent(wrapperName)
      }
  }
...
extension SDK {
  ...
  static let setFromFallbackXcodeprojBuildSettings: SignalProducer<Set<SDK>?, Never> =
...
extension SDK {
  ...
  static let setsFromJSONShowSDKsWithFallbacks: SignalProducer<Set<SDK>, Never> =
