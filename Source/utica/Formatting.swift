...
private func wrap(_ isColorful: Bool, wrap: Color.Wrap) -> (String) -> String {
  return { string in
    isColorful ? wrap.wrap(string) : string
  }
}
...
public struct ColorOptions: OptionsProtocol {
  ...
  public struct Formatting {
    ...
    init(_ isColorful: Bool) {
      ...
      self.bulletin = wrap(isColorful, wrap: Color.Wrap(foreground: .blue, style: .bold))
      self.bullets = bulletin("***") + " "
      self.url = wrap(isColorful, wrap: Color.Wrap(styles: .underlined))
      self.projectName = wrap(isColorful, wrap: Color.Wrap(styles: .bold))
      self.path = wrap(isColorful, wrap: Color.Wrap(foreground: .yellow))
    }
  ...
  public static func evaluate(_ mode: CommandMode) -> Result<ColorOptions, CommandantError<CarthageError>> {
    return evaluate(mode, additionalUsage: nil)
  }
...
extension ColorOptions.Formatting {
  subscript(_ index: OutdatedCommand.UpdateType?) -> Wrap {
    guard isColorful, let update = index else {
      return { $0 }
    }
