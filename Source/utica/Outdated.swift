...
public struct OutdatedCommand: CommandProtocol {
  enum UpdateType {
    ...
    static var legend: String {
      let header = "Legend — <color> • «what happens when you run `utica update`»:\n"
    ...
    return options.loadProject()
      ...
      .on(value: { outdatedDependencies in
        ...
        if !outdatedDependencies.isEmpty {
          utica.println(formatting.path("The following dependencies are outdated:"))

          for (project, current, applicable, latest) in outdatedDependencies {
            if options.outputXcodeWarnings {
              utica.println("warning: \(formatting.projectName(project.name)) is out of date (\(current) -> \(applicable)) (Latest: \(latest))")
            } else {
              ...
              utica.println(formatting.projectName(project.name) + " " + versionSummary)
            }
          }
        } else {
          utica.println("All dependencies are up to date.")
        }
      })
      .waitOnCommand()
  }
