...
public struct ArchiveCommand: CommandProtocol {
  ...
  // swiftlint:disable:next function_body_length
  public func archiveWithOptions(_ options: Options) -> SignalProducer<Void, CarthageError> {
    ...
    if !options.frameworkNames.isEmpty {
      frameworks = .init(value: options.frameworkNames.map {
        ($0 as NSString).appendingPathExtension("framework")!
      })
    } else {
    ...
    return frameworks.flatMap(.merge) { frameworks -> SignalProducer<Void, CarthageError> in
      ...
      SignalProducer<SDK, CarthageError>(SDK.knownIn2019YearSDKs)
        .flatMap(.merge) { platform -> SignalProducer<String, CarthageError> in
          SignalProducer(frameworks).map { framework in
            (platform.relativePath as NSString).appendingPathComponent(framework)
          }
        }
        ...
        .on(value: { path in
          utica.println(formatting.bullets + "Found " + formatting.path(path))
        })
        .collect()
        .flatMap(.merge) { paths -> SignalProducer<Void, CarthageError> in
          ...
          if Set(foundFrameworks) != Set(frameworks) {
            let error = CarthageError.invalidArgument(
              description: "Could not find any copies of \(frameworks.joined(separator: ", ")). "
                + "Make sure you're in the project's root and that the frameworks have already been built using 'utica build --no-skip-current'."
            )
          ...
          return zip(paths: paths, into: outputURL, workingDirectory: options.directoryPath).on(completed: {
            utica.println(formatting.bullets + "Created " + formatting.path(outputPath))
          })
        }
    }
  }
